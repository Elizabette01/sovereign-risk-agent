...
def run_many(n_episodes: int = 50, seed: int = 42) -> None:
    ...
    print("\n=== Episode Summary (Random Policy) ===")
    print(df_eps.describe(include="all"))

    default_rate = df_eps["terminated"].mean()
    print(f"\nApprox 'default' rate (terminated=True): {default_rate:.2%}")

    # Save results so we can plot later
    df_eps.to_csv("episode_summary_random.csv", index=False)
    df_all.to_csv("steps_random.csv", index=False)
    print("\nSaved: episode_summary_random.csv and steps_random.csv")
